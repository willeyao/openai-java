...
public class OpenAiService {
    ...
    public Flowable<ChatMessageAccumulator> mapStreamToAccumulator(Flowable<ChatCompletionChunk> flowable) {
        ...
        ChatResultMessage accumulatedMessage = new ChatResultMessage(ChatMessageRole.ASSISTANT.value(), null);

        return flowable.map(chunk -> {
            ChatResultMessage messageChunk = chunk.getChoices().get(0).getMessage();
