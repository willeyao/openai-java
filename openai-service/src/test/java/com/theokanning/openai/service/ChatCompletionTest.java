...
class ChatCompletionTest {
    ...
    String token = "7632e9f41f9a48eabeb4a009f25600eb";
    OpenAiService service = new OpenAiService(token);
    AzureOpenAiService azureOpenAiService = new AzureOpenAiService(token);

    @Test
    void createChatCompletion() {
        final List<ChatMessage> messages = new ArrayList<>();
        final ChatMessage systemMessage = new ChatTextMessage(ChatMessageRole.SYSTEM.value(), "You are a copilot");
        final ChatMessage userMessage = new ChatImageMessage(ChatMessageRole.USER.value(), "What the image says?", "https://pic.rmb.bdstatic.com/bjh/events/44e7124f9b7a0981d705f0dff85f662c4141.jpeg@h_1280");
        ...
        List<ChatCompletionChoice> choices = azureOpenAiService.createChatCompletion("chat608-4o", "2024-02-01", chatCompletionRequest).getChoices();
    ...
    void streamChatCompletion() {
        final List<ChatMessage> messages = new ArrayList<>();
        final ChatMessage systemMessage = new ChatTextMessage(ChatMessageRole.SYSTEM.value(), "You are a dog and will speak as such.");
        messages.add(systemMessage);
        ...
        azureOpenAiService.streamChatCompletion("chat608-4o", "2024-02-01", chatCompletionRequest).blockingForEach(chunks::add);
    ...
    void createChatCompletionWithFunctions() {
        ...
        final List<ChatMessage> messages = new ArrayList<>();
        final ChatMessage systemMessage = new ChatTextMessage(ChatMessageRole.SYSTEM.value(), "You are a helpful assistant.");
        final ChatMessage userMessage = new ChatTextMessage(ChatMessageRole.USER.value(), "What is the weather in Monterrey, Nuevo León?");
        messages.add(systemMessage);
        messages.add(userMessage);
        ...
        messages.add(new ChatTextMessage(choice.getMessage().getRole(), choice.getMessage().getContent()));
    ...
    void streamChatCompletionWithFunctions() {
        ...
        ChatResultMessage accumulatedMessage = service.mapStreamToAccumulator(service.streamChatCompletion(chatCompletionRequest))
                .blockingLast()
        ...
        messages.add(new ChatTextMessage(accumulatedMessage.getRole(), accumulatedMessage.getContent()));
        messages.add(callResponse);
        ...
        ChatResultMessage accumulatedMessage2 = service.mapStreamToAccumulator(service.streamChatCompletion(chatCompletionRequest2))
                .blockingLast()
                .getAccumulatedMessage();
