...
import com.theokanning.openai.client.azure.AzureAuthenticationInterceptor;
import com.theokanning.openai.client.azure.AzureOpenAiApi;
...
public class AzureOpenAiService {
    ...
    private static final String BASE_URL = "https://chat009.openai.azure.com/";
     ...
     */
    public AzureOpenAiService(final String token) {
        this(BASE_URL, token, DEFAULT_TIMEOUT);
    }

    public AzureOpenAiService(final String url, final String token) {
        this(url, token, DEFAULT_TIMEOUT);
    }
     ...
     */
    public AzureOpenAiService(final String url, final String token, final Duration timeout) {
     ...
     */
    public AzureOpenAiService(final AzureOpenAiApi api) {
        this.api = api;
     ...
     */
    public AzureOpenAiService(final AzureOpenAiApi api, final ExecutorService executorService) {
        this.api = api;
    ...
    public ChatCompletionResult createChatCompletion(String deploymentId, String apiVersion, ChatCompletionRequest request) {
        return execute(api.createChatCompletion(deploymentId, apiVersion, request));
    }

    public Flowable<ChatCompletionChunk> streamChatCompletion(String deploymentId, String apiVersion, ChatCompletionRequest request) {
        request.setStream(true);
        return stream(api.createChatCompletionStream(deploymentId, apiVersion, request), ChatCompletionChunk.class).skip(1);
    }
    ...
    public static OpenAiApi buildApi(String url, String token, Duration timeout) {
        ObjectMapper mapper = defaultObjectMapper();
        OkHttpClient client = defaultClient(token, timeout);
        Retrofit retrofit = defaultRetrofit(url, client, mapper);
    ...
    public static Retrofit defaultRetrofit(String url, OkHttpClient client, ObjectMapper mapper) {
