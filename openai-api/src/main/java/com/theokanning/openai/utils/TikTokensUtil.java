...
import com.theokanning.openai.completion.chat.ChatImageContent;
import com.theokanning.openai.completion.chat.ChatMessage;
import com.theokanning.openai.completion.chat.ChatTextContent;
...
public class TikTokensUtil {
    ...
    public static int tokens(String modelName, List<ChatMessage> messages) {
        ...
        for (ChatMessage msg : messages) {
            ...
            for (ChatContent chatContent : msg.getContent()) {
                if (chatContent instanceof ChatTextContent) {
                    ChatTextContent chatTextContent = (ChatTextContent) chatContent;
                    sum += tokens(encoding, chatTextContent.getText());
                } else if (chatContent instanceof ChatImageContent) {
                    ChatImageContent chatImageContent = (ChatImageContent) chatContent;
                    if (chatImageContent.getImageUrl() != null && chatImageContent.getImageUrl().getDetail() != null) {
                        sum += tokens(encoding, chatImageContent.getImageUrl().getDetail());
                    }
                }
            }
